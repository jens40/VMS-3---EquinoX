...
$rallydaten = explode("|",$mainconfig[$r_art.'_plaetze']);
	...
	if ($r_art == 'skr' or $r_art == 'sar' or $r_art == 'sbr' or $r_art == 'srr') $ausgelobt = (int)$ausgelobt + (int)$rallyinhalt[$zaehler];
	}
...
$rallyuser_output = $db->fetch($rallyuser);
    ...
    if ($r_art == 'dkr' or $r_art == 'dar' or $r_art == 'dbr') {
    $rallyuser_out['gewinn'] = number_format(((int)$mainconfig[$r_art.'_startpot'] / 100 * (int)$rallyinhalt[$platz]),$mainconfig['nach_koma'],",",".");
    }else{
    $rallyuser_out['gewinn'] = number_format((int)$rallyinhalt[$platz],0,",",".");
    }
